...
public class AdminEditarRepartidorActivity extends AppCompatActivity {
    ...
    ActivityResultLauncher<Intent> tomarFoto = registerForActivityResult(
            ...
            result -> {
                if (result.getResultCode() == Activity.RESULT_OK) {
                    ...
                    uri = Uri.parse(path);
                    Glide.with(AdminEditarRepartidorActivity.this).load(bitmap).diskCacheStrategy(DiskCacheStrategy.NONE).skipMemoryCache(true).into(imageView);
                }
            }
    );
    @Override
    protected void onResume() {
        ...
        Repartidor repartidor = intent.getSerializableExtra("repartidor") == null ? null : (Repartidor) intent.getSerializableExtra("repartidor");
        if(repartidor != null){
            ImageView imageView = findViewById(R.id.imageAgregarRepartidor);
            ...
            id = repartidor.getDni();
            if(uri == null){
                Glide.with(AdminEditarRepartidorActivity.this).load(storageReference.child("repartidores").child(repartidor.getDni()+"/photo.jpg")).diskCacheStrategy(DiskCacheStrategy.NONE).skipMemoryCache(true).into(imageView);
            }
            editNombre.setText(repartidor.getNombres());
            editApellidos.setText(repartidor.getApellidos());
            editDni.setText(repartidor.getDni());
            editDni.setEnabled(false);
            editTelefono.setText(repartidor.getNumero());
            editCorreo.setText(repartidor.getCorreo());
        }else{
            Toast.makeText(AdminEditarRepartidorActivity.this, "Error al cargar el producto", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_admin_editar_repartidor);
        //Navbar
        BottomNavigationView bottomNavigationView =  findViewById(R.id.bottonnav);
        navbar(bottomNavigationView, AdminEditarRepartidorActivity.this);
        //Tomar foto
        ...
        ((Button) findViewById(R.id.btnEditarRepartidor)).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                EditText editNombre = findViewById(R.id.editNombreRepartidor);
                EditText editApellidos = findViewById(R.id.editApellidoRepartidor);
                    ...
                    if(bool) {
                        ...
                        firebaseFirestore.collection("repartidores").document(repartidor.getDni()).set(repartidor).addOnSuccessListener(new OnSuccessListener<Void>() {
                            ...
                            public void onSuccess(Void unused) {
                                Intent intent = new Intent(AdminEditarRepartidorActivity.this,AdminListaRepartidoresActivity.class);
